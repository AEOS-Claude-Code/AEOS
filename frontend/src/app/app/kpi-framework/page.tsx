...
const STATUS_CONFIG: Record<string, { color: string; icon: any; label: string }> = {
  ...
  not_tracked: { color: "bg-surface-secondary text-fg-muted ring-border", icon: Clock, label: "Not Tracked" },
};
...
const PRIORITY_COLORS: Record<string, string> = {
  ...
  low: "bg-surface-inset text-fg-secondary",
};
...
function KPICard({ kpi }: { kpi: KPIItem }) {
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-border/60 bg-surface px-4 py-3 shadow-sm transition-all hover:shadow-md">
      <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${st.color} ring-1`}>
        <Icon size={13} />
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="text-xs font-bold text-fg">{kpi.name}</p>
          <span className={`rounded-full px-1.5 py-px text-2xs font-bold ${prColor}`}>{kpi.priority}</span>
        </div>
        <p className="mt-0.5 text-2xs text-fg-muted">{kpi.description}</p>
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          {kpi.current_value && (
            <span className="rounded-lg bg-surface-secondary px-2 py-0.5 text-2xs font-semibold text-fg ring-1 ring-border-light">
              Current: {kpi.current_value}
            </span>
          )}
          <span className="text-2xs text-fg-hint">Target: {kpi.target}</span>
          <span className="text-2xs text-fg-hint">| {kpi.frequency}</span>
        </div>
      </div>
    </div>
  );
...
function KPISection({ title, icon: SectionIcon, kpis, color }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      className="rounded-2xl border border-border/60 bg-surface shadow-lg shadow-slate-100/50">
      <button onClick={() => setExpanded(!expanded)}
        ...
        <div className="flex-1">
          ...
          <p className="text-2xs text-fg-muted">{kpis.length} KPIs · {onTrack} on track · {atRisk} at risk</p>
        </div>
  ...
  if (error || !report) {
    return (
      <div className="rounded-2xl border border-border bg-surface p-8 text-center">
        ...
        <p className="text-sm text-fg-secondary">{error || "No KPI framework available"}</p>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-fg">KPI Framework</h1>
            <p className="text-xs text-fg-muted">{report.total_kpis} KPIs across {4} categories</p>
          </div>
        </div>
      ...
      <StaggerGrid className="grid gap-4 sm:grid-cols-3">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="text-center">
            ...
            <p className="text-2xs text-fg-hint">KPI Health</p>
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="flex items-center gap-4 rounded-2xl border border-border/60 bg-surface p-5 shadow-lg shadow-slate-100/50">
          <div>
            <p className="text-2xl font-bold text-fg">{report.tracked_kpis}/{report.total_kpis}</p>
            <p className="text-xs text-fg-muted">KPIs Tracked</p>
            <div className="mt-1 h-1.5 w-24 overflow-hidden rounded-full bg-surface-inset">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <p className="mb-2 text-xs font-semibold text-fg-muted">Review Cadence</p>
          <div className="space-y-1">
            {Object.entries(report.review_cadence).map(([freq, items]) => (
              <div key={freq} className="flex items-center gap-2">
                <span className="w-16 text-2xs font-bold text-fg-secondary capitalize">{freq}</span>
                <span className="text-2xs text-fg-hint">{items.length} metrics</span>
              </div>
            ))}
          </div>
        </motion.div>
