...
export function CardSkeleton({ className = "" }: { className?: string }) {
  return (
    <div className={`animate-pulse rounded-xl border border-border bg-surface p-5 ${className}`}>
      <div className="mb-3 h-4 w-1/3 rounded bg-surface-inset" />
      ...
      <div className="mt-4 h-8 w-1/4 rounded bg-surface-secondary" />
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="animate-pulse">
        <div className="h-7 w-48 rounded bg-surface-inset" />
